/*
    ...
    Решение задачи 0685 с сайта acmp.ru
*/
...
int main() {
    int a, b, c, aa, bb, cc; read(a, b, c, aa, bb, cc);
    if (a < b) swap(a, b); if (a < c) swap(a, c); if (b < c) swap(c, b);
    if (aa < bb) swap(aa, bb); if (aa < cc) swap(aa, cc); if (bb < cc) swap(cc, bb);
    doans(a * aa + b * bb + c * cc);
}